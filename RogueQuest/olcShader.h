...
#define DEFAULT_PS "void main()                                            \n" \
	...
	public:
		...
		void SetAttribute(const std::string& name, const olc::vf2d& value);
	...
	protected:
		...
		std::map<std::string, GLint> m_UniformRefs;
	...
	public: // Simplified Interface
		...
		void DrawRotatedDecal(const olc::vf2d& pos, olc::Decal* decal, const float fAngle, const olc::vf2d& center, const olc::vf2d& scale = { 1.0f,1.0f }, const olc::Pixel& tint = olc::WHITE);
	...
	private:
		...
		olc::Effect ConstructShader(const std::string& sVS, const std::string& sPS, const std::vector<std::tuple<std::string, std::string, std::string>>& attribs);
...
#ifdef OLC_PGEX_SHADER_IMPLEMENTATION
...
namespace olc
{
	...
	void Effect::SetAttribute(const std::string& name, float value) {
		if (auto loc = m_UniformRefs.find(name); loc != m_UniformRefs.end()) {
			s_Shade->locUseProgram(m_nID);
			s_Shade->locUniform1f(loc->second, value);
		}
	}

	void Effect::SetAttribute(const std::string& name, int value) {
		if (auto loc = m_UniformRefs.find(name); loc != m_UniformRefs.end()) {
			s_Shade->locUseProgram(m_nID);
			s_Shade->locUniform1i(loc->second, value);
		}
	}

	void Effect::SetAttribute(const std::string& name, const olc::vf2d& value) {
		if (auto loc = m_UniformRefs.find(name); loc != m_UniformRefs.end()) {
			s_Shade->locUseProgram(m_nID);
			s_Shade->locUniform2f(loc->second, value.x, value.y);
		}
	}
	...
	olc::Effect Shade::ConstructShader(const std::string& sVS, const std::string& sPS, const std::vector<std::tuple<std::string, std::string, std::string>>& attribs)
	{
		olc::Effect effect;
		...
		// Configure Shader Uniform References
		for (const auto& attrib : attribs) {
			const std::string& name = std::get<0>(attrib);

			effect.m_UniformRefs.insert_or_assign(name, locGetUniformLocation(sh_id, name.c_str()));
		}
	...
	olc::Effect Shade::MakeEffect(const olc::EffectConfig& premade)
	{
		...
		olc::Effect effect = ConstructShader(sVertexShader, sPixelShader, premade.vAttributes);
	...
	void Shade::OnBeforeUserCreate()
	{
		...
		locUniform1f = OGL_LOAD(locUniform1f_t, glUniform1f);
	...
	void Shade::DrawRotatedDecal(const olc::vf2d& pos, olc::Decal* decal, const float fAngle, const olc::vf2d& center, const olc::vf2d& scale, const olc::Pixel& tint)
	{
		SetSourceDecal(decal, 0);
		...
		for (int i = 0; i < 4; i++)
		{
			...
			di.pos[i] = di.pos[i] * slotTarget[0].vInvSize * 2.0f - olc::vf2d(1.0f, 1.0f);
		}

		Render(di);
	}

	void Shade::DrawPartialRotatedDecal(const olc::vf2d& pos, olc::Decal* decal, const float fAngle, const olc::vf2d& center, const olc::vf2d& source_pos, const olc::vf2d& source_size, const olc::vf2d& scale, const olc::Pixel& tint)
	{
		SetSourceDecal(decal, 0);
